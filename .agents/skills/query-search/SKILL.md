...
description: Execute one web search query for job discovery. Takes a query string and title filters, runs a single WebSearch, parses results, applies filtering, and returns only a flat JSON array of job listings.
---

# query-search

Executes exactly **one** search query and returns filtered job listings.

## Input (passed by the parent agent)

- `query`: The search query string (e.g. `site:jobs.ashbyhq.com "Senior" "React" remote`)
...
1. **Search**: Run `WebSearch` with the provided `query`.
2. **Parse**: For each result, create a candidate object:
