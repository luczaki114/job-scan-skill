...
Prints results as a JSON array to stdout; writes nothing to disk.
"""
...
def _get(url: str, company_name: str):
    ...
    try:
        ...
        print(f"  [WARN] {company_name}: {exc}", file=sys.stderr)
        return None
...
def main():
    ...
    for company in companies:
        ...
        print(f"[{ats}] {name} ...", file=sys.stderr)
        ...
        all_jobs.extend(new)
        print(f"  {len(new)} match{'es' if len(new) != 1 else ''}", file=sys.stderr)

    # Print only the JSON array to stdout so the parent skill can capture it.
    print(json.dumps(all_jobs, indent=2, ensure_ascii=False))

    if skipped:
        print(f"\nSkipped ({len(skipped)} — route to websearch-scan or playwright-scan):", file=sys.stderr)
        for s in skipped:
